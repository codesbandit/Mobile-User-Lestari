...
import 'package:lestar_user/features/address/domain/services/address_service_interface.dart';
...
class AddressController extends GetxController implements GetxService {
  ...
  Future<void> getAddressList({
    ...
    if (dataSource == DataSourceEnum.local) {
      addressList = await addressServiceInterface.getList(
  ...
  void filterAddresses(String queryText) {
    if (_addressList != null) {
      _addressList = addressServiceInterface.filterAddresses(
        _addressList!,
        queryText,
      );
      update();
  ...
  Future<ResponseModel> updateAddress(
    ...
    _isLoading = true;
    update();
    ResponseModel responseModel = await addressServiceInterface.update(
      addressModel.toJson(),
      addressId!,
    );
    if (responseModel.isSuccess) {
