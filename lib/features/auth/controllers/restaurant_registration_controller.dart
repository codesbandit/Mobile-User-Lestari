import 'package:lestar_user/features/business/controllers/business_controller.dart';
import 'package:lestar_user/features/business/domain/models/package_model.dart';
...
import 'package:lestar_user/features/auth/domain/services/restaurant_registration_service_interface.dart';
...
class RestaurantRegistrationController extends GetxController
  ...
  Future<void> pickFile(int index, MediaData mediaData) async {
    FilePickerResult? result = await restaurantRegistrationServiceInterface
        .picFile(mediaData);
    if (result != null) {
  ...
  Future<void> getZoneList() async {
    ...
    _zoneIds = null;
    _zoneList = await Get.find<DeliverymanRegistrationController>()
        .getZoneList();
    if (_zoneList != null) {
      setLocation(
        LatLng(
  ...
  void setLocation(
    LatLng location, {
    bool forRestaurantRegistration = false,
    int? zoneId,
  }) async {
  ...
  void pickImage(bool isLogo, bool isRemove) async {
    if (isRemove) {
      ...
      if (isLogo) {
        _pickedLogo = await restaurantRegistrationServiceInterface
            .picLogoFromGallery();
      } else {
        _pickedCover = await restaurantRegistrationServiceInterface
            .picLogoFromGallery();
      }
      update();
    }
  ...
  Future<void> registerRestaurant(
    ...
    Response? response = await restaurantRegistrationServiceInterface
        .registerRestaurant(
          data,
          _pickedLogo,
          _pickedCover,
  ...
  Future<void> getPackageList({bool isUpdate = true}) async {
    _packageModel = await restaurantRegistrationServiceInterface
        .getPackageList();
    if (isUpdate) {
      update();
    }
