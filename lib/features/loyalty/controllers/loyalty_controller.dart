import 'package:lestar_user/features/loyalty/domain/services/loyalty_service_interface.dart';
...
class LoyaltyController extends GetxController implements GetxService {
  ...
  Future<void> getLoyaltyTransactionList(String offset, bool reload) async {
    ...
    if (!_offsetList.contains(offset)) {
      ...
      WalletModel? transactionModel = await loyaltyServiceInterface
  ...
  Future<void> convertPointToWallet(int point) async {
    _isLoading = true;
    update();
    Response response = await loyaltyServiceInterface.convertPointToWallet(
      point,
    );
