...
class AddFundBottomSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double? previousAmount =
        walletAmount - double.parse(notificationModel.data?.amount ?? '0.0');
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Center(child: Text('add_fund'.tr, style: robotoBold)),
          ),
          ...
          Container(
            ...
            child: Column(
              children: [
                buildRow(
                  'wallet_previous_amount'.tr,
                  PriceConverter.convertPrice(previousAmount),
                ),
                buildRow(
                  'credit_amount'.tr,
                  PriceConverter.convertPrice(
                    double.parse(notificationModel.data?.amount ?? '0.0'),
                  ),
                ),
                buildRow(
                  'current_amount'.tr,
                  PriceConverter.convertPrice(walletAmount),
                ),
              ],
            ),
          ),
