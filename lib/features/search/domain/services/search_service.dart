...
import 'package:lestar_user/features/search/domain/models/search_suggestion_model.dart';
import 'package:lestar_user/features/search/domain/repositories/search_repository_interface.dart';
import 'package:lestar_user/features/search/domain/services/search_service_interface.dart';
  ...
  List<Restaurant>? sortRestaurantSearchList(
    ...
    if (isAvailableRestaurants || isDiscountedRestaurants) {
      ...
      if (isDiscountedRestaurants) {
        searchRestaurantList.removeWhere(
          (restaurant) => restaurant.discount == null,
        );
      }
    }
    if (sortIndex != -1) {
    ...
    if (restaurantPriceSortIndex != -1) {
      if (restaurantPriceSortIndex == 0) {
        ...
        searchRestaurantList!.sort(
          (a, b) => b.minimumOrder!.compareTo(a.minimumOrder!),
        );
      }
    }
