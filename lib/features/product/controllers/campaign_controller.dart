...
import 'package:lestar_user/features/product/domain/services/campaign_service_interface.dart';
...
class CampaignController extends GetxController implements GetxService {
  ...
  Future<void> getBasicCampaignList(bool reload) async {
    if (_basicCampaignList == null || reload) {
      _basicCampaignList = await campaignServiceInterface
  ...
  Future<void> getBasicCampaignDetails(int? campaignID) async {
    _campaign = null;
    _campaign = await campaignServiceInterface.getCampaignDetails(
      campaignID.toString(),
    );
    update();
  }
