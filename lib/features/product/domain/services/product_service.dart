...
import 'package:lestar_user/features/product/domain/repositories/product_repository_interface.dart';
...
class ProductService implements ProductServiceInterface {
  ...
  @override
  Future<List<Product>?> getPopularProductList({
  ...
  Future<Product?> getProductDetails({
    ...
    return await productRepositoryInterface.get(
      id.toString(),
      ...
      for (int index = 0; index < variations.length; index++) {
        ...
        for (int i = 0; i < variations[index].variationValues!.length; i++) {
          variationsStock[index].add(
            variations[index].variationValues![i].currentStock,
          );
        }
      }
    }
  ...
  int _quantityLimitCheck(
    ...
    if (_haveSelectedVariation(selectedVariations) &&
        stockType != 'unlimited' &&
        !isCampaign) {
      minimumStock = _minimumVariationStock(
        selectedVariations,
        variationsStock,
      );
    }
  ...
  @override
  int isExistInCartForBottomSheet(
