...
import 'package:lestar_user/features/address/widgets/address_card_widget.dart';
...
import 'package:lestar_user/features/checkout/widgets/order_successfull_dialog_widget.dart';
import 'package:lestar_user/features/checkout/widgets/partial_pay_dialog.dart';
import 'package:lestar_user/features/coupon/controllers/coupon_controller.dart';
...
class CheckoutController extends GetxController implements GetxService {
  ...
  void setInstruction(int index, {bool willUpdate = true}) {
    _selectedInstruction = checkoutServiceInterface.selectInstruction(
      index,
  ...
  void changesMethod() {
    ...
    informationControllerList = checkoutServiceInterface
        .generateTextControllerList(methodInformation);
    informationFocusList = checkoutServiceInterface.generateFocusList(
      methodInformation,
    );
  ...
  bool isRestaurantClosed(
    ...
    return Get.find<RestaurantController>().isRestaurantClosed(
      dateTime,
      active,
      schedules,
    );
  ...
  void setPreferenceTimeForView(
    String time,
    bool instanceOrder, {
    bool isUpdate = true,
  }) {
    _preferableTime = checkoutServiceInterface.setPreferenceTimeForView(
      time,
      instanceOrder,
    );
    if (isUpdate) {
  ...
  Future<bool> checkBalanceStatus(
    double totalPrice, {
    double discount = 0,
    ...
    if (Get.find<RestaurantController>().restaurant != null) {
      if (Get.find<AddressController>().addressList != null) {
        ...
        for (
          ...
          if (Get.find<AddressController>().addressList![index].zoneIds!
  ...
  Future<String> placeOrder(
    ...
    if (response.statusCode == 200) {
      ...
      Response notificationResponse = await checkoutServiceInterface
