...
import 'package:lestar_user/features/order/domain/repositories/order_repository_interface.dart';
...
class OrderService implements OrderServiceInterface {
  ...
  List<SubscriptionScheduleModel>? processSchedules(Response response) {
    ...
    if (response.body['subscription_schedules'] != null) {
      response.body['subscription_schedules'].forEach(
        (schedule) =>
  ...
  @override
  List<OnlineCart> prepareOnlineCartList(
    int? restaurantZoneId,
    List<OrderDetailsModel> orderedFoods,
    List<Product> foods,
  ) {
    List<OnlineCart> onlineCartList = [];
    if (AddressHelper.getAddressFromSharedPref()!.zoneIds!.contains(
      restaurantZoneId,
    )) {
      for (int i = 0; i < orderedFoods.length; i++) {
        for (int j = 0; j < foods.length; j++) {
          if (orderedFoods[i].foodDetails!.id == foods[j].id) {
            onlineCartList.add(
              _sortOutProductAddToCard(
                orderedFoods[i].variation,
                foods[j],
                orderedFoods[i],
                getOnlineCart: true,
              ),
  ...
  @override
  List<CartModel> prepareOfflineCartList(
    int? restaurantZoneId,
    List<OrderDetailsModel> orderedFoods,
    List<Product> foods,
  ) {
  ...
  dynamic _sortOutProductAddToCard(
    List<Variation>? orderedVariation,
    Product currentFood,
          ...
          for (
            ...
            if (selectedVariations[i][j]!) {
              variations[variations.length - 1].values!.label!.add(
                currentFood.variations![i].variationValues![j].level,
              );
              optionIds.add(
                currentFood.variations![i].variationValues![j].optionId,
              );
            }
          }
        }
    ...
    for (var addon in currentFood.addOns!) {
      for (int i = 0; i < orderDetailsModel.addOns!.length; i++) {
        if (orderDetailsModel.addOns![i].id == addon.id) {
          ...
          addOnIdWithQtnList.add(
            cart.AddOn(
              id: addon.id,
              quantity: orderDetailsModel.addOns![i].quantity,
            ),
          );
        }
      }
