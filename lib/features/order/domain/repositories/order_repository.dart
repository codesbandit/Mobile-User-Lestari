...
import 'package:lestar_user/features/order/domain/models/refund_model.dart';
...
class OrderRepository implements OrderRepositoryInterface {
  ...
  Future<List<Product>?> getFoodsFromFoodIds(List<int?> ids) async {
    List<Product>? foods;
    Response response = await apiClient.postData(
      AppConstants.productListWithIdsUri,
      {'food_id': jsonEncode(ids)},
    );
    if (response.statusCode == 200) {
      foods = [];
  ...
  Future<ResponseModel> updateSubscriptionStatus(
    ...
    Response response = await apiClient
        .postData('${AppConstants.subscriptionListUri}/$subscriptionID', {
          '_method': 'put',
          'status': status,
          'note': note,
