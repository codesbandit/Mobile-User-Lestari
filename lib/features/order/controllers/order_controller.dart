...
import 'package:lestar_user/features/order/domain/models/delivery_log_model.dart';
import 'package:lestar_user/features/order/domain/models/order_cancellation_body.dart';
import 'package:lestar_user/features/order/domain/models/order_details_model.dart';
import 'package:lestar_user/features/order/domain/models/order_model.dart';
import 'package:lestar_user/features/order/domain/models/pause_log_model.dart';
import 'package:lestar_user/features/order/domain/models/subscription_schedule_model.dart';
import 'package:lestar_user/features/order/domain/services/order_service_interface.dart';
import 'package:lestar_user/helper/address_helper.dart';
import 'package:lestar_user/helper/auth_helper.dart';
import 'package:lestar_user/helper/date_converter.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  Future<void> getRunningSubscriptionOrders(
    int offset, {
    bool notify = true,
  }) async {
    if (offset == 1) {
    ...
    if (!_runningSubscriptionOffsetList.contains(offset)) {
      ...
      PaginatedOrderModel? paginatedOrderModel = await orderServiceInterface
          .getRunningSubscriptionOrderList(offset);
      if (paginatedOrderModel != null) {
        if (offset == 1) {
          _runningSubscriptionOrderList = [];
        }
  ...
  Future<void> getOrderCancelReasons() async {
    List<CancellationData>? reasons = await orderServiceInterface
        .getCancelReasons();
    if (reasons != null) {
      _orderCancelReasons = [];
  ...
  Future<bool> switchToCOD(
    ...
    _isLoading = true;
    update();
    ResponseModel responseModel = await orderServiceInterface.switchToCOD(
      orderID,
    );
    if (responseModel.isSuccess) {
      if (points != null) {
  ...
  Future<void> submitRefundRequest(String note, String? orderId) async {
    if (_selectedReasonIndex == 0) {
      ...
      Map<String, String> body = orderServiceInterface.prepareReasonData(
        note,
        orderId,
        _refundReasons![selectedReasonIndex]!,
      );
  ...
  Future<bool> cancelOrder(int? orderID, String? cancelReason) async {
    ...
    Get.back();
    if (responseModel.isSuccess) {
      OrderModel? orderModel = orderServiceInterface.findOrder(
        _runningOrderList,
        orderID,
      );
  ...
  Future<void> reOrder(
    List<OrderDetailsModel> orderedFoods,
    int? restaurantZoneId,
  ) async {
    _isLoading = true;
    update();

    List<int?> foodIds = orderServiceInterface.prepareFoodIds(orderedFoods);
    List<Product>? responseFoods = await orderServiceInterface
        .getFoodsFromFoodIds(foodIds);
    if (responseFoods != null) {
      ...
      _canReorder = AddressHelper.getAddressFromSharedPref()!.zoneIds!.contains(
        restaurantZoneId,
      );
      ...
      if (_canReorder) {
        await Get.find<CartController>().reorderAddToCart(onlineCartList).then((
          statusCode,
        ) {
          if (statusCode == 200) {
