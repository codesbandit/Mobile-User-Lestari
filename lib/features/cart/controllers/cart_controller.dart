...
import 'package:lestar_user/common/widgets/cart_snackbar_widget.dart';
...
import 'package:lestar_user/features/cart/domain/services/cart_service_interface.dart';
...
class CartController extends GetxController implements GetxService {
  ...
  void setAvailableIndex(int index, {bool willUpdate = true}) {
    _notAvailableIndex = cartServiceInterface.setAvailableIndex(
      index,
      _notAvailableIndex,
    );
    if (willUpdate) {
  ...
  Future<void> addToCartOnline(
    OnlineCart onlineCart, {
    CartModel? existCartData,
    bool fromDirectlyAdd = false,
  }) async {
    _isLoading = true;
    update();
  ...
  Future<int?> _addMultipleCartItemOnline(List<OnlineCart> cartList) async {
    _isLoading = true;
    update();
    Response response = await cartServiceInterface.addMultipleCartItemOnline(
      cartList,
    );
    if (response.statusCode == 200) {
      List<OnlineCartModel> onlineCartList = [];
  ...
  Future<void> updateCartOnline(
    OnlineCart onlineCart, {
    CartModel? existCartData,
  }) async {
    _isLoading = true;
    update();
    Response response = await cartServiceInterface.updateCartOnline(
      onlineCart,
