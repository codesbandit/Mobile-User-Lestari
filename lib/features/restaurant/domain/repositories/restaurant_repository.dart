...
import 'package:lestar_user/api/local_client.dart';
...
class RestaurantRepository implements RestaurantRepositoryInterface {
  ...
  Future<List<Restaurant>?> _getLatestRestaurantList(
    String type, {
    DataSourceEnum? source,
  }) async {
    ...
    switch (source!) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
          '${AppConstants.latestRestaurantUri}?type=$type',
        );
        if (response.statusCode == 200) {
  ...
  Future<List<Restaurant>?> _getPopularRestaurantList(
    String type, {
    DataSourceEnum? source,
  }) async {
    ...
    switch (source!) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
          '${AppConstants.popularRestaurantUri}?type=$type',
        );
        if (response.statusCode == 200) {
  ...
  Future<List<Restaurant>?> _getRecentlyViewedRestaurantList(
    ...
    switch (source!) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
          '${AppConstants.recentlyViewedRestaurantUri}?type=$type',
        );
        if (response.statusCode == 200) {
