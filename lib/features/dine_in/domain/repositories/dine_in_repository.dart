...
class DineInRepository implements DineInRepositoryInterface {
  ...
  Future<DineInModel?> getRestaurantList({
    ...
    Response response = await apiClient.getData(
      '${AppConstants.dineInRestaurantListUri}?offset=$offset&limit=10&sort_by=${isDistance
          ? 'distance'
          : isRating
          ? 'rating'
          : ''}&veg=${isVeg ? 1 : 0}&non_veg=${isNonVeg ? 1 : 0}&discount=${isDiscounted ? 1 : 0}&cuisine=$selectedCuisines',
    );
