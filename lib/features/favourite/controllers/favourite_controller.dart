...
import 'package:lestar_user/features/favourite/domain/services/favourite_service_interface.dart';
...
class FavouriteController extends GetxController implements GetxService {
  ...
  void addToFavouriteList(
    ...
    Response response = await favouriteServiceInterface.addFavouriteList(
      isRestaurant ? restaurantId : product!.id,
      isRestaurant,
    );
  ...
  void removeFromFavouriteList(int? id, bool isRestaurant) async {
    _isDisable = true;
    update();
    Response response = await favouriteServiceInterface.removeFavouriteList(
      id,
      isRestaurant,
    );
    if (response.statusCode == 200) {
